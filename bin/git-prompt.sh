...
#################################################################### WHO_WHERE
        ...
        if [[ -n $id  || -n $host ]] ;   then
                ...
                color_who_where="$user_id_color($color_who_where$user_id_color) $colors_reset"
        else
                color_who_where=''
        fi
...
prompt_command_function() {
        ...
        MAX_GIT_CHARS=$((COLUMNS-2))
#        GIT_PS1="$colors_reset$rc$head_local$color_who_where$dir_color$nl$cwd$tail_local$colors_reset"
        GIT_PS1="$colors_reset$color_who_where$rc$head_local$dir_color$nl$cwd$tail_local$colors_reset"
        PS1="\n┌$GIT_PS1\n└$prompt_char"
