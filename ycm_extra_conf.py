...
def IsCppSourceFile(filename):
  extension = os.path.splitext(filename)[1]
  return extension in [ '.cpp', '.cxx', '.cc' ]
...
def GetFlagsForFile(filename):
  ...
  if IsCppSourceFile(filename):
    ...
    for extension in SOURCE_EXTENSIONS:
      replacement_file = basename + extension
      if os.path.exists(replacement_file):
        flags = GetFlagsForFile(replacement_file)
        if flags:
          break;
    # Defaults to CFLAGS if cannot match the header with a source file.
    flags = CFLAGS
...
def FlagsForFile(filename, **kwargs):
  if database:
    ...
    final_flags = GetFlagsForFile(filename)
